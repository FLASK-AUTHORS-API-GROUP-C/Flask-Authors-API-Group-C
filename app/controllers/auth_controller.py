...
#Request response cycle 
    ...
    try:
         hashed_password = bcrypt.generate_password_hash(password)# Hashing password to encrypt password,to avoid unauthorised access


        #Creating the user
         new_author = Author(first_name=first_name,last_name=last_name,password=hashed_password,email=email,contact=contact,biography=biography, user_type=type)
         ...
         return jsonify({
              ...
              'author':{
                   ...
                   'type': new_author.user_type,
                   'created': new_author.created_at,
...
def login():
    ...
    try:
       ...
       user = Author.query.filter_by(email=email.lower()).first()
       if user:
...
@jwt_required(refresh=True)
def refresh():
    identity = get_jwt_identity()  # Already a string if stored properly
    access_token = create_access_token(identity=identity)  # Just reuse it
