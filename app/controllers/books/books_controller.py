...
# Books blueprint
books = Blueprint('books', _name_,url_prefix='/api/v1/books')
...
def createNewBook():
    ...
    if type == 'books' and not description:
        return jsonify({"error":"Enter the book's description."}),HTTP_400_BAD_REQUEST
    
    
    if not validators.email:
        return jsonify({"error":"Email is not valid."}),HTTP_400_BAD_REQUEST
    
    if books.query.filter_by(email = email).first() is not None:
        return jsonify({"error":"Email address already in use."}),HTTP_409_NOT_CONFLICT
    
    if books.query.filter_by(contact=contact).first() is not None:
